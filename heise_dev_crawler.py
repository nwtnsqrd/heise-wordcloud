...
def init_table(conn):
  c = conn.cursor()
  sql = 'CREATE TABLE IF NOT EXISTS frontpage_stats_dev(id integer primary key autoincrement, date text, title text, synopsis text)'
...
def insert_into_table(conn, teaser_titles, teaser_synopsis):
  ...
  sql = 'INSERT INTO frontpage_stats_dev (date, title, synopsis) VALUES (?, ?, ?)'
...
def get_all_rows(conn):
  cur = conn.cursor()
  cur.execute('SELECT * from frontpage_stats_dev')
...
def prepare_wordcloud(conn):
  cur = conn.cursor()
  cur.execute('SELECT title, synopsis from frontpage_stats_dev WHERE date = \'%s\'' % strftime('%Y-%m-%d'))
  ...
  for row in range(len(rows)):
    rows[row] = rows[row].replace("'", '').replace('(', '').replace(')', '').replace(',', '').replace('"', '').replace(':', '').replace('[', '').replace(']', '').replace('?', '').replace('Der', '').replace('Die', '').replace('Das', '').replace('Auf', '').replace('Mit', '').replace('Ein', '').replace('Eine', '').replace('Software', '').replace('Developer', '')
...
def create_wordcloud(prepared_list):
  ...
  wc.to_file('./images_dev/wordcloud_%s.png' % strftime('%Y-%m-%d-%H-%M-%S'))
...
def main():
  ...
  teaser_titles = news_content.find_all('h1', { 'class' : 'akwa-article-teaser__title' })
  teaser_synopsis = news_content.find_all('p', { 'class' : 'akwa-article-teaser__synopsis' })
